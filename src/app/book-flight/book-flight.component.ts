...
export class BookFlightComponent implements OnInit {
  ...
  onSubmit(flightForm: any) {
    ...
    this.departDate = this.datePipe.transform(newDate, "dd-MM-yyyy");
    let departDay=this.day[newDate.getDay()];
    this.result = this.data.filter((f: Flight) => (f.fromPlace === fromPlace.toLowerCase() && f.toPlace === toPlace.toLowerCase() && (f.scheduledDays.includes('daily')|| f.scheduledDays.includes(departDay))));
    if (!this.oneway) {
      let returnDate = new Date(flightForm.form.get('return').value);
      if(this.datePipe.transform(returnDate, "dd-MM-yyyy")){
        this.returnDate = this.datePipe.transform(returnDate, "dd-MM-yyyy");
      }
